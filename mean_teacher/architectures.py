...
class CNN(nn.Module):
    ...
    def __init__(self, num_classes=10, isL2=False, double_output=False):
        """13-layer CNN model used in mean-teacher.
            13 = 4 (conv + pool) + 4 + 4 + 1 (fc)

        Args:
            num_classes (int, optional): number of classes. Defaults to 10.
            isL2 (bool, optional): use L2 norm after feature extractor. Defaults to False.
            double_output (bool, optional): use double output trick in mean-teacher. Defaults to False.
        """
        ...
        self.conv1a = weight_norm(nn.Conv2d(3, 128, 3, padding=1))  # note weight_norm trick
    ...
    def forward(self, x, debug=False):
        ...
        x = x.view(-1, 128)
        if self.isL2:  # normalize feature vector before fc, mt not use this
            x = F.normalize(x)

        if self.double_output:
            return self.fc1(x), self.fc2(x), x  # 2 logits and 1 feature
        else:
            return self.fc1(x), x  # # 1 logits and 1 feature
